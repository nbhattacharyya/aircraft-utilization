export const set = (program) => program.command("set <name> <value>", "Set the value of a secret", (yargs) => yargs
    ...
    try {
        ...
        const { edgeSites, sites, placeholderSites, functions, sitesWithPrefetch, functionsWithPrefetch, } = await Config.restart([args.name]);
        ...
        if (functionsWithPrefetch.length > 0) {
            Colors.line(Colors.primary(`➜ `), `Redeploy the functions with "prefetchSecret" enabled to use the new secret`);
        }
        await exit();
    }
    catch (e) {
